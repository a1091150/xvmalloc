 ...
 */
static u32 find_block(struct xv_pool *pool,
                      u32 size,
                      struct page **page,
                      u32 *offset)
{
 ...
 */
static void insert_block(struct xv_pool *pool,
                         struct page *page,
                         u32 offset,
                         struct block_header *block)
{
    ...
    if (block->link.next_page) {
        nextblock =
            get_ptr_atomic(block->link.next_page, block->link.next_offset);
 ...
 */
static void remove_block(struct xv_pool *pool,
                         struct page *page,
                         u32 offset,
                         struct block_header *block,
                         u32 slindex)
{
    ...
    if (block->link.prev_page) {
        tmpblock =
            get_ptr_atomic(block->link.prev_page, block->link.prev_offset);
 ...
 */
int xv_malloc(struct xv_pool *pool, u32 size, struct page **page, u32 *offset)
{
    int error;
...
void xv_free(struct xv_pool *pool, struct page *page, u32 offset)
{
    ...
    if (tmpblock && test_flag(tmpblock, BLOCK_FREE)) {
         ...
         */
        if (tmpblock->size >= XV_MIN_ALLOC_SIZE) {
            remove_block(pool, page, offset + block->size + XV_ALIGN, tmpblock,
